...
//validate form for check error of form pass
  const validateForm =  ()=>{ 
    ...
    if(inputField.repass === '' ){ 
      formValid =false ; 
      setErrField(prevState=> ({ 
        ...prevState,repassErr: 'Please Enter Confirm Password !!'
      }))
    }
    if(inputField.repass !=='' && inputField.repass !== inputField.password ){ 
      formValid =false ; 
      setErrField(prevState=> ({ 
        ...prevState,repassErr: 'Password was not match !!'
      }))
    }
    return formValid ; 
  }
          ...
          <hr  className='hrForgot'/>
            <div className="passDiv">
                <input 
                      ...
                      className="passInput" />
                { 
                      errField.otpCodeErr.length > 0  && <span className='error-pass'>{errField.otpCodeErr} </span>
                }
            </div>
            <div className="passDiv">
                <input 
                      ...
                      className="passInput" />
                { 
                      errField.otpCodeErr.length > 0  && <span className='error-pass'>{errField.passwordErr} </span>
                }
            </div>
            <div className="passDiv">
                <input 
                      ...
                      className="passInput" />
                { 
                      errField.repassErr.length > 0  && <span className='error-pass'>{errField.repassErr} </span>
                }
            </div>
            
          <div className="buttonForgot">
            {/* Link to login page */}
            <button className="Exit"><Link to={"/login"} className="linkExit">Back</Link></button>
            <button className="Find" type ="submit" onClick={submitButton}>Change Password</button>
          </div>
          </form> 
